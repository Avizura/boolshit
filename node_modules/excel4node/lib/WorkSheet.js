...
function toXML(){
	...
	*/
	if(Object.keys(thisWS.cols).length > 0){
		if(thisWS.sheet.cols instanceof Array === false){
			thisWS.sheet.cols=[];
		}
	...
	*/
	if(thisWS.hyperlinks && thisWS.hyperlinks.length>=0){
		thisWS.sheet.hyperlinks = [];
		thisWS.hyperlinks.forEach(function(cr){
			thisWS.sheet.hyperlinks.push({'hyperlink':{'@ref':cr.ref,'@r:id':'rId' + cr.id}});
		});
	};
	...
	*/
	...
	excelOrder.forEach(function(k){
		if(k=='autoFilter' && thisWS.sheet[k]){
			thisWS.sheet.sheetPr['@enableFormatConditionsCalculation']='1';
			thisWS.sheet.sheetPr['@filterMode']='1';

			var refParts = thisWS.sheet[k][0]['@ref'].split(':');
			var cell = refParts[1].toExcelRowCol();
			refParts[1] = cell.col.toExcelAlpha() + thisWS.sheet.sheetData.length;
			thisWS.sheet[k][0]['@ref'] = refParts.join(':');
		}
		if(thisWS.sheet[k]){
			var def = {};
			def[k] = thisWS.sheet[k];
